...
public class Classificator {
    ...
    public Classificator() {
        try {
            ...
            Path labelsPath = Paths.get(this.getClass().getResource("/trained_model/imagenet_comp_graph_label_strings.txt").toURI());
    ...
    private List<String> findPredictedLabel(float[][] prediction) {
        ...
        for (int i = 0; i < batchSize; i++) {
            ...
            result.add("I think that this is a " + labels.get(maxValueIndex) + " with " + (prediction[i][maxValueIndex] * 100) + "% confidence level!");
        }
        return result;
    }
}
